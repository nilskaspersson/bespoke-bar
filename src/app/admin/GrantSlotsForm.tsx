...
export function GrantSlotsForm({ defaultOrgId }: Props) {
	...
	async function action(formData: FormData) {
		...
		const promise = grantSlotsManual({ orgId, amount, source, note });

		toast.promise(promise, {
			loading: "Granting slots…",
			success: () => ({
				message:
					amount >= 0
						? `Granted ${amount} slots to ${orgId}`
						: `Removed ${Math.abs(amount)} slots from ${orgId}`,
			}),
			error: (err) => ({
				message: "Could not grant slots",
