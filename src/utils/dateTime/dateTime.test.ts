...
describe("formatRelativeTime", () => {
	...
	describe("static labels for recent times", () => {
		it('shows "Just now" for times less than 2 minutes ago', () => {
			...
			expect(formatRelativeTime(date, 30, mockNow)).toBe("Just now");
		});

		it('shows "A few minutes ago" for times 2-9 minutes ago', () => {
			const date = new Date("2025-06-22T11:57:00Z"); // 3 minutes ago
			expect(formatRelativeTime(date, 30, mockNow)).toBe("A few minutes ago");
		});
	});
	...
	describe("absolute time formatting", () => {
		it("shows absolute time for dates beyond default threshold (30 days)", () => {
			...
			const result = formatRelativeTime(date, 30, mockNow);
		...
		it("respects custom threshold", () => {
			...
			expect(formatRelativeTime(date, 30, mockNow)).toBe("7 days ago");

			const result = formatRelativeTime(date, 5, mockNow);
	...
	describe("edge cases", () => {
		...
		it("handles exactly 10 minutes", () => {
			...
			expect(formatRelativeTime(date, 30, mockNow)).toBe("10 minutes ago");
		});

		it("handles exactly 1 hour", () => {
			const date = new Date("2025-06-22T11:00:00Z"); // exactly 1 hour ago
			expect(formatRelativeTime(date, 30, mockNow)).toBe("1 hour ago");
		});
