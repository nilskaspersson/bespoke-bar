...
import { getRecipeSlotUsage } from "../billing/getRecipeSlotUsage";
import { cacheEvents } from "../cache";
import { enrichIngredients } from "../ingredients/enrichIngredients";
import { rateLimit } from "../rateLimit";
import { enrichRecipes } from "./enrichRecipes";
import { extractIngredientsToCreate } from "./schema";
import {
	insertIngredientsInTransaction,
	replaceLinesInTransaction,
	upsertRecipeInTransaction,
} from "./utils/transactionHelpers";

export async function upsertRecipesWithLines(
	auth: Auth,
	...
	const [recipes, createdIngredients] = await db.transaction(async (tx) => {
		...
		for (const o of userInputRecipes) {
			...
			/**
			 * Step 4: Replace all lines for the recipe with provided lines
			 * TODO: Merge with `processedRecipe` for a more complete return value?
			 */
			await replaceLinesInTransaction(
				tx,
				processedRecipe.id,
				o.lines,
				ingredientIdsByName,
